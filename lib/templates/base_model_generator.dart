...
class BaseModelGenerator extends FileGenerator {
	...
	List<String> usedMethods = ['getTableName', 'getColumnNames', 'getColumns', 'getColumnTypes', 'getColumnType', 'hasColumn', 'getPrimaryKeys', 'getPrimaryKey', 'isAutoIncrement', 'fetchSingle', 'fetch', 'fromResult', 'castInts', 'insertIntoPool', 'retrieveFromPool', 'removeFromPool', 'flushPool', 'setPoolEnabled', 'getPoolEnabled', 'getAll', 'doCount', 'doDelete', 'doSelect', 'doSelectOne', 'doUpdate'];
...
''');
			...
			if (rawMethodName.toLowerCase() != methodName.toLowerCase() && !usedMethods.contains('get${rawMethodName}')) {
...
''');
		for (Column field in fields) {
			if (field.isNotNull() && !field.isAutoIncrement() && field.getDefaultValue() == null && !field.isPrimaryKey() && !(['created', 'updated'].contains(field.getName()))) {
				result.write('''
